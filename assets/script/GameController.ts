import { _decorator, Asset, AudioClip, BoxCollider, BoxCollider2D, Camera, Canvas, CCBoolean, cclegacy, Collider, Color, Component, debug, DebugMode, director, DistanceJoint2D, EColliderType, ERigidBody2DType, ERigidBodyType, EventMouse, EventTouch, find, game, geometry, gfx, Graphics, HingeConstraint, HingeJoint2D, ICollisionEvent, Input, instantiate, IQuatLike, JsonAsset, Layers, lerp, MeshRenderer, misc, Node, ParticleSystemComponent, PhysicsSystem, PlaneCollider, PointToPointConstraint, Prefab, profiler, Quat, renderer, resources, RigidBody, RigidBody2D, SceneGlobals, Scheduler, Size, Sprite, tween, Tween, UITransform, Vec2, Vec3, Widget } from 'cc';
...
import { ModelToUIConverter } from './ModelToUIConverter';
import { UImanager } from './UImanager';
import { ShopWin } from './ShopWin';
import { FloorRandomColor } from './FloorRandomColor';
import { AudioMgr } from './AudioMgr';
import { CraneLod } from './CraneLod';
...
export class GameController extends Component {
    ...
    private _curSoundBg = 1;
    private twGuide: Tween<Color>
    private isShowLodState: boolean;
    public getFloorNum(): number {
        return this._floorNodes.length;
    }
    /**获取最后一层楼高度 */
    ...
    public initMainUI(mainUI: MainUI) {
        this.mainUI = mainUI;
    ...
    public initData(createComplete?: Function) {
        ...
        this.SkyObj = new Node();
        this.SkyObj.setParent(director.getScene())
        ...
        this.createNewBox(createComplete);
        this.craneHolder.setWorldPosition(this.craneHolderPos)
        this.cityModelChange(true);
        this.skyBoxHandler();
    }
    start() {
        console.log("GameControllerStart")

        this.worldCamera = director.getScene().getChildByName("MainCamera").getComponent(Camera)
        // this.worldCamera.clearFlags= renderer.scene.ProbeClearFlag.SKYBOX;
        // this.worldCamera.node.active = true;
        this.assetMgr = AssetMgr.instance;
        // this.initMainUI();
        var UIManager = director.getScene().getChildByName("UIManager")
        if (UIManager) {

            // this.PopUILayer = UIManager.getChildByName("PopUILayer")
    ...
    subHp() {
        ...
        if (this.curHp < 1) {
            ...
            if (!this.UIManager.showWin("ReGameWin", this._floorNodes.length, this.mainUI.maxRate, this.mainUI.money, this.again.bind(this), this.advFunc.bind(this))) {
                this.isStart = false;
                this.showLod(true)
            } else {
                this.isStart = false;
    ...
    again() {
        ...
        this.initData(this.onStartFunc.bind(this));
        if (this._dropBox.node) {
            this.assetMgr.removeInstant(this._dropBox.node)
        }
        // this.onStartFunc();
    }
    advFunc() {
    ...
    update(deltaTime: number) {
        ...
        if (!this.isNotMove && this._floorNodes && this._floorNodes.length > 0) {
            this._holderEurAngle.x = Math.sin(this.roTime * 2) * this.rockMaxAngle;
            this.holderRock.eulerAngles = this._holderEurAngle;
            this._holderPos.z = 25.57 + this.craneMoveMaxX * Math.sin(this.roTime * 2)
            ...
            this.holderRock.eulerAngles = Vec3.ZERO;
            this._holderPos.z = 25.57
        }
    ...
    private frozenFloor(rightBody: RigidBody): void {
        ...
        var preFloor = this.getPreFloor()
        if (preFloor) {
            var collider = preFloor.getComponent(Collider)
            if (collider) {
        ...
        var len = this._floorNodes.length;
        if (len > 10) {
            let floor = this._floorNodes[len - 11]
            let craneGrow = this.craneGrows[len - 11]
            let floorandom = floor.getComponent(FloorRandomColor)
            if (floorandom) {
                floorandom.lodShow();
            }
            var craneLod = craneGrow.getComponent(CraneLod)
            if (craneLod) {
                craneLod.lodShow();
            }
            floor.active = craneGrow.active = false;
        }

        // this._floorNodes[this._floorNodes.length - 11].active = false;

        // rightBody.node.eulerAngles = Vec3.ZERO;
    }
    showLod(isShow: boolean) {
        var len = this._floorNodes.length;
        this.isShowLodState = isShow;
        if (len > 10) {
            for (let i = 0; i < len - 10; i++) {
                let floor = this._floorNodes[i]
                let craneGrow = this.craneGrows[i]
                floor.active = isShow;
                craneGrow.active = isShow;
        ...
        if (lenFloor > 0) {
            ...
            if (lenFloor > 1) {
                ...
                if (offset >= 0) {
                    ...
                    this.curHp--
                    AudioMgr.inst.playRemoteSound("tower_discarded", true)
                    this.scheduleOnce(() => {
                        this.dropFailHandle(lastFloor)
                    }, 1)
                } else {

                    this.frozenFloor(rightBody)
                    this.getLastFloor()
                    if (lastFloor.name == "shop") {
                        var shopItem = GameConfig.getShopItemByIdx(this._floorNodes.length - 1)
                        if (shopItem) {
                            this.UIManager.showWin("ShopWin", shopItem.items)
                        }
                    }
                    ...
                    this.mainUI.setCoinBasePos(ModelToUIConverter.convertModelToUI(lastFloor, this.UIManager.node))
                    if (this.checkIsPerfactPos(lastFloor.position.x, preFloor.position.x)) {//完美命中
                        ...
                        var randColor = lastFloor.getComponent(FloorRandomColor)
                        if (randColor) {
                            randColor.showOutLight(true, lastFloor)
                        }
                    } else {
                        AudioMgr.inst.playRemoteSound("clonks", true)
                        console.log("命中")
                        this.rockHandler(forceDir);
                        this.mainUI.addMoney(GameConfig.config.getMoneyByQuality(this.mainUI.towerQuality) * (1 - Math.abs(offset) / this.dropDistance))
                    }
                ...
                AudioMgr.inst.playRemoteSound("breaking_wooden_crate", true)
                this.focuseLastFloorPos(lastFloor)
                this.frozenFloor(rightBody)
                this.createNewBox();
                this.showBeginGuide(false)
                var config = AssetMgr.instance.RES_CONFIG;
                if (config.isSupportAdv || config.isSupportShare) {
                    UImanager.showWin("AwardNoticeWin")
                }
            }
        }
    }
    ...
    createCraneGrow(isTween: boolean) {
        this.assetMgr.getRes("CraneGrow", Prefab, "components").then(data => {
            ...
            var len = this.craneGrows.length
            var posY = obj.parent.worldPosition.y
            if (len > 1) {
                posY = this.craneGrows[len - 2].worldPosition.y;
            }
            // var lastCrane = this.craneGrows.length[len - 1]
            pos.y = posY//obj.parent.worldPosition.y + (this.craneGrows.length - 1) * this.floorHeight;
    ...
    skyBoxHandler() {
        ...
        if (alpha >= 1) {//已冲破第一层天空
            ...
            this.changeSkyBoxAlpha(this.skyBox2, 1, true)
        }
        if (numFloor == 180) {
    ...
    cityModelChange(isShowCity: boolean) {
        if (isShowCity) {
            if (!this.cityNode || (this.cityNode && this.cityNode.name != this.CITYMODEL_NAME)) {
                ...
                this.assetMgr.createPrefab(this.CITYMODEL_NAME, "components", Vec3.ZERO, director.getScene()).then(data => {
            ...
            if (this.cityNode && this.cityNode.name != this.EARTHMODEL_NAME) {
                this.assetMgr.removeInstant(this.cityNode)
                this.assetMgr.createPrefab(this.EARTHMODEL_NAME, "components", Vec3.ZERO, director.getScene()).then(data => {
                    this.cityNode = data as Node;
                })
            }
        }
    }
    createNewBox(call?: Function) {
        // this.assetMgr.getRes("BaseBox", Prefab, "components").then(data => { this.createBox(data) })
        this._dropBoxPos = null;
        var item = GameConfig.getShopItemByIdx(this._floorNodes.length)
        var createName = ""
        if (item) {
            createName = item.prefabName;
        } else {
            createName = this.getFloorName()
        }
        this.assetMgr.getRes(createName, Prefab, "components").then(data => { this.createFloorBox(data); if (call) { call() } })
    }
    createFloorBox(box: Prefab) {
        if (box) {
            var obj = this.assetMgr.instantiate(box);
            if (obj) {
                var floorColor = obj.getComponent(FloorRandomColor)
                if (floorColor) {
                    floorColor.quality = this.mainUI.towerQuality;
                }
                var rig = obj.getComponent(RigidBody);
    ...
    createFrog(pos: Vec3, isPerfect: boolean = false) {
        this.assetMgr.getRes(isPerfect ? "frogParticle" : "frogParticle2", Prefab, "components").then(data => { this.createFrogData(data, pos) })
    }
    createFrogData(frog: Prefab, pos: Vec3) {
        if (frog) {
            ...
            if (obj) {
                ...
                obj.setWorldPosition(pos)
                this.scheduleOnce(() => {
                    this.assetMgr.removeInstant(obj)
                }, 2)
                // pa.play()
            }
        }
    ...
    onClickMainUI(touch: EventTouch, event) {
        ...
        if (this._isCanCreateNext && this._dropBox && this._dropBox.node != this.getLastFloor()) {
            if (this.isEditState) {
            ...
            if (dropFloor) {
                ...
                tween(this.hold).to(0.15, { scale: new Vec3(1, 0.5, 1) }).to(0.15, { scale: Vec3.ONE }).start();
    ...
    onStartFunc() {
        this._curSoundBg = 1;
        AudioMgr.inst.playRemoteSound("bg1", false, this.soundBgRound.bind(this))
        this.isStart = true;
        this.canCreateFunc();
        // this.mainUI.showNotice(false);
        var tw = this.resetCameraPos(this._dropBox.node);
        if (tw) {
            tw.start();
        }
        this.mainUI.showGuide("点击任意地方下落", 1)
        if (!this.firstFloorGuideSquar) {
            this.assetMgr.createPrefab("firstFloorGuideSquar", "components", new Vec3(0, 0.1, 0), director.getScene()).then(data => {
                this.firstFloorGuideSquar = (data as Node);
    ...
    soundBgRound() {
        this._curSoundBg++
        if (this._curSoundBg > 4) {
            this._curSoundBg = 4;
        }
        AudioMgr.inst.playRemoteSound("bg" + this._curSoundBg, false)
    }
    showBeginGuide(bol: boolean) {
        ...
        if (bol && this.firstFloorGuideSquar) {
    ...
    checkSkyObjCreate() {
        ...
        if (skyObj) {
            ...
            list.forEach(item => {
                this.assetMgr.getRes("skyNode/" + item.name, Prefab).then(data => {
                    if (item.sound) {
                        AudioMgr.inst.playRemoteSound(item.sound, true);
                    }
                    var obj = this.assetMgr.instantiate(data);
                    if (obj) {
                        if (this.isEditState) {
                        ...
                        if (item.isFollowCamera) {
                            obj.setPosition(item.pos[0], item.pos[1], item.pos[2])
                            obj.setParent(this.SkyObj)
                        } else {
    ...
    public addFloor(num: number) {
        this.createFloorList(num + this._floorNodes.length)
    }
    async createFloorListItem() {
        ...
        return this.assetMgr.getRes(this.getFloorName(), Prefab, "components").then(data => {
            var obj = this.assetMgr.instantiate(data);
            if (obj) {
                var scale = this._dropBox.node.getWorldScale();
                obj.setScale(scale)
